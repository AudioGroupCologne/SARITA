...
data.dir    = pwd;%[];       % full path of the directory for saving the data (if this is empty the directory is selected in a GUI)

% select which data to save
data.IR     = true;     % save IR after deconvolution (variable 'ir')
data.raw    = true;    % save recorded sweep and excitation signal (variables 'raw' and 'sweep')
...
s.subSonic = false;  % apply 20 Hz high pass to filter out noise
...
% Level and averaging
s.levelOut_dB   = -12; % output level in dB FS
